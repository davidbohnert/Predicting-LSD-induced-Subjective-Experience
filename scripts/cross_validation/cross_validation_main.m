...
for bIdx = 1:length(BEHAV_SCALES)
    ...
    if contains(behavName, 'LSD_MEQ30'), diffMatrixName = [diffMatrixName '_LAM_LPM']; end
    
    diffBehavName = behavName;
    if contains(diffMatrixName, 'difference'), diffBehavName = [behavName '_difference']; end
    
    diffBehavFile = fullfile(inputPath, 'behav', [diffBehavName '.mat']);
    diffConnFile = fullfile(inputPath, 'connectomes', [diffMatrixName '.mat']);
    
    if isfile(diffBehavFile) && isfile(diffConnFile)
        ...
        results = permutation_test_cv(diffMatrixName, diffBehavName, diffMats, diffBehav, K_FOLDS, BASE_THR, [], outputTxt, results);
        results = permutation_test_cv(diffMatrixName, diffBehavName, diffMats, diffBehav, K_FOLDS, BASE_THR, covars, outputTxt, results);
    end
